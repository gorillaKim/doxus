...
fn main() {
    ...
    let total_embeddings: i64 = conn.query_row("SELECT count(*) FROM chunk_embeddings", [], |r| r.get(0)).unwrap();
    let total_fts: i64 = conn.query_row("SELECT count(*) FROM chunks_fts", [], |r| r.get(0)).unwrap();
