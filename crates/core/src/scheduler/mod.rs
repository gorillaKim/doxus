...
pub use schedule::{Executor, Schedule, ScheduledJob};
...
impl SchedulerManager {
    pub fn new(conn: crate::db::DbPool, indexer: Arc<IndexingService>) -> Self {
        Self {
            conn,
            indexer,
        ...
        for job in due_jobs {
            ...
            tokio::spawn(async move {
                let result = match job.executor {
                    Executor::System => {
                        executor::execute_system(&job.action, &job.action_config, &indexer_clone)
                            .await
                    }
                    Executor::Agent => {
                        if let Some(handler) = handler_opt {
                            handler
                                .execute_agent(&job.job_name, &job.action, &job.action_config)
                                .await
                        } else {
                            JobResult {
                                success: false,
    ...
    pub fn ensure_defaults(&self) {
        ...
        if !has_co_refs_prune {
            let default_job = ScheduledJob {
                ...
                description: Some(
                    "오래되고 빈도가 낮은 공동 참조 관계 데이터를 정해진 주기로 삭제합니다."
                        .to_string(),
                ),
                executor: Executor::System,
