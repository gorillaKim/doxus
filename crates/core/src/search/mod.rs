...
impl SearchEngine {
    ...
    pub fn new_fts_only(pool: crate::db::DbPool) -> Self {
        Self {
            conn: pool,
            embedder: Arc::new(crate::embedding::NoOpEmbedder)
            ...
            let mut sub_chunk_counts: Vec<usize> = Vec::with_capacity(sub_batch.len());
            {
                ...
                tokio::task::spawn_blocking(move || -> Result<(), SearchError> {
                    ...
                    for (doc_idx, req) in sub_batch.into_iter().enumerate() {
                        ...
                        let doc_chunks = &sub_chunks_moved
                            [current_chunk_offset..current_chunk_offset + num_chunks];
                        let doc_embeddings = &sub_flat_embeddings
                            [current_chunk_offset..current_chunk_offset + num_chunks];
...
pub(crate) fn index_document_sync(
    ...
    if content.trim().is_empty() && title.trim().is_empty() {
        tracing::debug!(
            "[Search-Engine][index] Skipping empty document: source_doc_id={}",
            source_doc_id
        );
        return Ok(());
    }
    ...
    conn.execute(
        "DELETE FROM document_aliases WHERE document_id = ?1",
        [doc_id],
    )?;
    ...
    for (k, v) in &meta.metadata {
        conn.execute("INSERT OR REPLACE INTO document_metadata (document_id, key, value) VALUES (?1, ?2, ?3)", params![doc_id, k, v.to_string()])?;
    }
    ...
    for (i, chunk) in chunks.iter().enumerate() {
        ...
        if is_reference {
            conn.execute("UPDATE chunks SET content = NULL WHERE id = ?1", [chunk_id])?;
        }
...
fn fts_search_sync(conn: &Connection, query: &SearchQuery) -> Result<Vec<SearchHit>, SearchError> {
    ...
    let (project_filter, projects_next_param) = if query.project_ids.is_empty() {
        ...
        (
            ...
            next_param + query.project_ids.len(),
        )
    };
    ...
    if let Some(v) = query.created_after {
        date_filter.push_str(&format!(" AND d.created_at >= ?{}", next_param));
    ...
    if let Some(v) = query.updated_after {
        date_filter.push_str(&format!(" AND d.updated_at >= ?{}", next_param));
        params_vec.push(Box::new(v));
        next_param += 1;
    }
    ...
    let keywords: Vec<String> = query
        ...
        .collect();
    let highlighter =
        Highlighter::new(&keywords).unwrap_or_else(|_| Highlighter::new(&[]).unwrap());
            ...
            if let Some(ref path_str) = hit.file_path {
                ...
                if path.exists() {
                    if let (Some(start), Some(end)) = (hit.start_byte, hit.end_byte) {
                        if let Ok(res) =
                            highlighter.highlight_file(path, start as usize, end as usize, 50)
                        {
...
fn vector_search_sync(
    ...
    let mut params_vec: Vec<Box<dyn rusqlite::ToSql>> =
        vec![Box::new(emb_bytes.to_vec()), Box::new(k)];
...
fn assemble_context_sync(
    ...
    let (_target_idx, target_content, start_byte, end_byte): (
        i32,
        Option<String>,
        Option<i64>,
        Option<i64>,
    ...
    let actual_content = if let Some(c) = target_content {
        c
    } else if let (Some(path_str), Some(start), Some(end)) =
        (hit.file_path.as_ref(), start_byte, end_byte)
    {
        let path = std::path::Path::new(path_str);
        if path.exists() {
            let mut file =
                std::fs::File::open(path).map_err(|e| SearchError::Join(e.to_string()))?;
            use std::io::{Read, Seek, SeekFrom};
            file.seek(SeekFrom::Start(start as u64))
                .map_err(|e| SearchError::Join(e.to_string()))?;
