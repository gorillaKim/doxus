...
impl DocumentService {
    ...
    pub async fn fetch_full_content(
        ...
        project_name: &str,
        source_doc_id: &str,
    ) -> Result<doxus_plugin_sdk::RawDocument, ServiceError> {
        tracing::info!(
            "[DS] Starting fetch_full_content for doc: {:?}",
            source_doc_id
        );
        ds_log(&format!(
            "[DS] Starting fetch_full_content for doc: {}",
        ...
        tracing::info!("[DS] Project metadata loaded.");
                    ...
                    if let Ok(content) = std::fs::read_to_string(path) {
                        ...
                        let db_result = if let Some(path) = &self.db_path {
                            crate::db::open_readonly(path).ok().and_then(|c| {
                                c.query_row(
                                    meta_query,
                                    params![project_name, source_doc_id, path_str],
                                    |r| {
        ...
        let plugin_id = PluginManager::normalize_id(&source_type);
        {
            ...
            if let Some(path) = &self.db_path {
                ds_log("[DS] Opening independent DB for cache check...");
                tracing::info!("[DS] Opening independent DB for cache check...");
                let conn = crate::db::open_readonly(path).map_err(ServiceError::Db)?;
                ds_log("[DS] DB opened. Checking ContentCache...");
                tracing::info!("[DS] DB opened. Checking ContentCache...");
                let cache = ContentCache::new(&conn);
                if let Ok(Some(data_json)) = cache.get_full(&plugin_id, source_doc_id) {
                    if let Ok(doc) =
                        serde_json::from_str::<doxus_plugin_sdk::RawDocument>(&data_json)
                    {
                        ds_log("[DS] Cache HIT.");
                        tracing::info!("[DS] Cache HIT.");
                        return Ok(doc);
                    }
                }
                ds_log("[DS] Cache MISS.");
                tracing::info!("[DS] Cache MISS.");
            } else {
                ds_log("[DS] Getting connection for cache check...");
                tracing::info!("[DS] Getting connection for cache check...");
                let conn = self.conn.read_conn()?;
                let cache = ContentCache::new(&conn);
                if let Ok(Some(data_json)) = cache.get_full(&plugin_id, source_doc_id) {
                    if let Ok(doc) =
        ...
        ds_log("[DS] Proceeding to Remote Fetch Strategy...");
        tracing::info!("[DS] Proceeding to Remote Fetch Strategy...");
        if let Some(pm) = &self.plugin_manager {
            tracing::info!("[DS] Getting source for plugin: {}", plugin_id);
            if let Some(mut source) = pm.get_source(&plugin_id) {
                ds_log("[DS] Plugin source acquired. Preparing config...");
                tracing::info!("[DS] Plugin source acquired. Preparing config...");
                ...
                ds_log("[DS] Calling inject_keychain_auth...");
                tracing::info!("[DS] Calling inject_keychain_auth...");
                inject_keychain_auth(&plugin_id, &mut plugin_config, &mut plugin_secrets).await;
                ds_log("[DS] inject_keychain_auth done.");
                tracing::info!("[DS] inject_keychain_auth done.");
                ...
                if let Err(e) = source.initialize(plugin_config, plugin_secrets).await {
                    ...
                    return Err(ServiceError::Plugin(format!(
                ...
                ds_log(&format!(
                    "[DS] Calling source.fetch_document for {}",
                    source_doc_id
                ));
                ...
                match fetch_result {
                    Ok(Ok(doc)) => {
                        ...
                        ds_log("[DS] Fetch SUCCESS");
                        tracing::info!("[DS] Fetch SUCCESS");
                        return Ok(doc);
                    }
                    Ok(Err(e)) => {
                    ...
                    Err(_) => {
                        ds_log("[ERR] [DS] Fetch TIMEOUT (30s)");
                        tracing::error!("[DS] Fetch TIMEOUT (30s)");

                        if let Some(path) = &self.db_path {
                            if let Ok(conn) = crate::db::open(path) {
                                crate::observability::persist_audit(
                                    &conn,
                                    &crate::observability::AuditEvent::DocumentFetchError {
                                        project: project_name.to_string(),
                                        doc_id: source_doc_id.to_string(),
                        ...
                        return Err(ServiceError::Plugin(
                            "Plugin fetch timed out after 30s".to_string(),
                        ));
                    }
                }
            }
        ...
        Err(ServiceError::NotFound(format!(
            "Content for {} is unavailable (no local file, no cache, and plugin fetch failed)",
            source_doc_id
        )))
    }
    ...
    pub async fn create_document(
        ...
        let pm = self
            .plugin_manager
            .as_ref()
            .ok_or_else(|| ServiceError::Plugin("Plugin manager not available".into()))?;
        let mut source = pm.get_source(&plugin_id).ok_or_else(|| {
            ServiceError::Plugin(format!("Source plugin {} not found", plugin_id))
        })?;
        ...
        source
            .initialize(plugin_config, plugin_secrets)
            .await
            .map_err(|e| {
        ...
        source
            .create_document(title, content, folder, metadata.as_ref())
            .await
            .map_err(|e| {
...
mod tests {
    ...
    use doxus_plugin_sdk::{
        Capabilities, ChangeSet, DocumentStream, FetchAllOpts, FetchChangesOpts, HealthStatus,
        PluginConfig, PluginError, PluginMetadata, PluginSecrets, RawDocument, SourceDocId,
    };
    ...
    async fn test_fetch_full_content_initializes_plugin() {
        ...
        assert_eq!(
            captured.fields.get("base_url").and_then(|v| v.as_str()),
            Some("http://test")
        );
    }
    ...
    async fn test_fetch_full_content_under_exclusive_lock() {
        ...
        // 1. Setup DB with project and cache data
        {
            ...
            cache
                .set_full("mock", "d1", "Cached Content", &data_json, 60)
                .unwrap();
        }
