...
async fn mock_v2_basics(server: &MockServer, space_key: &str, space_id: &str, space_name: &str) {
    ...
    Mock::given(method("GET"))
        .and(path(format!("/api/v2/spaces/{}/pages", space_id)))
        .respond_with(ResponseTemplate::new(200).set_body_json(&empty_list))
        .mount(server)
        .await;
    Mock::given(method("GET"))
        .and(path(format!("/api/v2/spaces/{}/folders", space_id)))
        .respond_with(ResponseTemplate::new(200).set_body_json(&empty_list))
        .mount(server)
        .await;
}
...
async fn fetch_all_paginates_with_cursor() {
    ...
    assert_eq!(
        ...
        Some("/api/v2/pages?cursor=page2&limit=25")
    );
...
async fn health_check_ancestor_only_config_returns_healthy() {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    Mock::given(method("GET"))
        .and(path("/api/v2/pages/4667998225"))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
