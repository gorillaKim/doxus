...
pub(crate) fn get_graph_data_impl(conn: &rusqlite::Connection) -> Result<GraphData, String> {
    ...
    let doc_nodes: Vec<GraphNode> = stmt
    ...
    let tags: Vec<String> = stmt
        .query_map([], |r: &rusqlite::Row<'_>| r.get(0))
        .map_err(|e| e.to_string())?
        .filter_map(|r| r.ok())
        .collect();
    ...
    let mut stmt = conn
        .prepare("SELECT source_id, target_id FROM document_links WHERE target_id IS NOT NULL")
        .map_err(|e| e.to_string())?;
    let doc_links: Vec<GraphLink> = stmt
        .query_map([], |r: &rusqlite::Row<'_>| {
            let sid: i64 = r.get(0)?;
            let tid: i64 = r.get(1)?;
            Ok(GraphLink {
    ...
    let mut stmt = conn
        ...
        .map_err(|e| e.to_string())?;
    let tag_links: Vec<GraphLink> = stmt
        .query_map([], |r: &rusqlite::Row<'_>| {
            let sid: i64 = r.get(0)?;
