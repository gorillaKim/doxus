...
pub(crate) fn increment_view_count_impl(
...
pub(crate) fn get_top_documents_impl(
    conn: &rusqlite::Connection,
    limit: usize,
) -> Result<Vec<TopDocument>, String> {
    let mut stmt = conn
        .prepare(
...
pub(crate) fn get_document_content_impl(
    ...
    let (doc_id, title): (i64, String) = conn
        .query_row(
...
mod tests {
    ...
    fn toggle_project_status_updates_status() {
        ...
        let status: String = conn
            .query_row("SELECT status FROM projects WHERE name = 'proj'", [], |r| {
                r.get(0)
            })
            .unwrap();
            ...
            if !types.is_empty() {
                ...
                if let Ok(mut stmt) = conn.prepare(&sql) {
                    let params: Vec<&dyn rusqlite::ToSql> =
                        types.iter().map(|s| s as &dyn rusqlite::ToSql).collect();
        ...
        if let Some(ref names) = project_names {
            if !names.is_empty() {
                let placeholders = names
                    ...
                    .join(",");
                let sql = format!(
                    "SELECT id FROM projects WHERE name IN ({}) AND status='active'",
                    placeholders
                );
                if let Ok(mut stmt) = conn.prepare(&sql) {
                    let params: Vec<&dyn rusqlite::ToSql> =
        ...
        for chunk in doc_ids.chunks(50) {
            ...
            for row in rows.flatten() {
                ...
                let metadata: serde_json::Value =
                    serde_json::from_str(&row.6).unwrap_or(serde_json::json!({}));
...
pub async fn index_project(
    ...
    let embedder = state.embedder.read().await.clone();
    let engine = std::sync::Arc::new(SearchEngine::with_embedder(state.conn.clone(), embedder));
    ...
    let message = if total == 0 {
        if is_full {
            "문서가 없는 프로젝트이거나 인덱싱에 실패했습니다".to_string()
        } else {
...
pub async fn trigger_reindex(
    ...
    state
        ...
        .record_external_trigger(
            ...
            None,
            Some(format!(
                "Manual {}re-index of all projects started",
                if is_full { "full " } else { "" }
            )),
        )
        .await;
    ...
    for name in names {
        let _ = state
            .sync_manager
            .trigger_full_indexing_by_name(&name, is_full)
            .await;
    }
...
pub async fn get_document_content(
    ...
    let doc = if let Some(ref pname) = project_name {
        ...
        if force_refresh.unwrap_or(false) {
            service
                .refresh_content(pname, &file_path)
                .await
                .map_err(|e| format!("문서 새로고침 실패: {e}"))?
        } else {
            service
                .fetch_full_content(pname, &file_path)
                .await
        ...
        tauri::async_runtime::spawn(async move {
            ...
            if let Some((pid, strategy)) = project_info {
                    ...
                    .await
                {
                    doxus_core::log_d!(
                        "commands",
                        "[JIT-Indexer] App background indexing triggered for: {} (ID: {})",
                        doc_clone.title.as_deref().unwrap_or("Untitled"),
                        doc_clone.id.0
                    );
                        ...
                        .is_ok()
                    {
                        ...
                        doxus_core::log_d!(
                            "commands",
                            "[JIT-Indexer] Event emitted: document-indexed for {}",
                            doc_id_for_log
                        );
                    }
                }
    ...
    let db_meta = {
        let conn = state.conn.get().map_err(|e| e.to_string())?;
        let row: Option<(Option<String>, Option<i64>, Option<i32>)> = conn
            .query_row(
    ...
    let tags = {
        ...
        if let Ok(mut stmt) = conn.prepare(sql) {
            if let Ok(rows) = stmt.query_map(rusqlite::params![project_name, doc.id.0], |r| {
                r.get::<_, String>(0)
            }) {
...
pub fn list_all_documents_impl(
    ...
    let docs: Vec<_> = stmt
        .query_map(
            rusqlite::params![limit_val as i64, offset_val as i64],
            |r| {
    ...
    doxus_core::log_d!(
        "commands",
        "[Core] list_all_documents: found {} documents (limit: {}, offset: {})",
        count,
        limit_val,
        offset_val
    );
